...
exports.get_expenses = async (page_number, page_limit, username, category, payment_type, type, start, end) => {
    const skips = (page_number - 1)*page_limit;
    return await executeQuery(db, async (db) => await db.collection(expenses_collection).find(
        {username: username, payment_type: {$regex: payment_type}, category: {$regex:category}, type: {$regex:type}, date: {"$gte": start, "$lte": end}}).sort({date: -1}).skip(skips).limit(page_limit).toArray());
};

exports.get_expenses_sum = async (page_number, page_limit, username, category, payment_type, type, start, end) => {
    const skips = (page_number - 1)*page_limit;
...
exports.get_expenses_by_month = async (username, month, page_number, page_limit, category, payment_type, type) => {
    const skips = (page_number - 1)*page_limit;
    return await executeQuery(db, async (db) => await db.collection(expenses_collection).find(
        { "$expr": { "$eq": [{ "$month": new Date() }, month] }, username: username, payment_type: {$regex: payment_type}, category: {$regex:category}, type: {$regex:type}}).sort({date: -1}).skip(skips).limit(page_limit).toArray());
};

exports.get_expenses_sum_month = async (username, month, page_number, page_limit, category, payment_type, type) => {
    const skips = (page_number - 1)*page_limit;
